...
class KeyCRMAPI:
    ...
    def update_lead_status(self, lead_id, status_id):
        """
        Оновлює статус ліда у воронці через PUT /pipelines/cards/{ID}

        Документація: https://help.keycrm.app/uk/process-automation-api-and-more/how-to-set-up-the-application-transfer-script

        Args:
            lead_id: ID ліда (картки) в KeyCRM
            status_id: ID нового статусу
                - 13 для Grand Opening Party (заходи)
                - 4 для місячних підписок

        Returns:
            dict: Відповідь від API або None у разі помилки
        """
        ...
        try:
            logger.info(f"🔄 Оновлення статусу ліда {lead_id} на статус {status_id}")
            logger.info(f"URL: {url}")
            logger.info(f"Payload: {payload}")
            ...
            result = response.json()
            logger.info(f"✅ Статус ліда {lead_id} успішно оновлено на {status_id}")
            logger.info(f"Відповідь: {response.text}")

            return result

        except requests.exceptions.HTTPError as e:
            logger.error(f"❌ HTTP помилка при оновленні статусу ліда: {e}")
            if hasattr(e, 'response') and e.response is not None:
            ...
            logger.error(f"❌ Помилка при оновленні статусу ліда: {e}")
            import traceback
